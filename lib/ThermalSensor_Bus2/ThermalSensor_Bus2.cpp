...
ThermalSensor_Bus2::ThermalSensor_Bus2()
{
...
uint8_t ThermalSensor_Bus2::begin(uint8_t address)
{
...
void ThermalSensor_Bus2::setUnit(temperature_units unit)
{
	_defaultUnit = unit; // Store the unit into a private member
}

uint8_t ThermalSensor_Bus2::read()
{
...
uint8_t ThermalSensor_Bus2::readRange()
{
...
uint8_t ThermalSensor_Bus2::readObject2()
{
	int16_t rawObj;
...
uint8_t ThermalSensor_Bus2::readAmbient()
{
	int16_t rawAmb;
...
uint8_t ThermalSensor_Bus2::readMax()
{
	int16_t toMax;
...
uint8_t ThermalSensor_Bus2::readMin()
{
	int16_t toMin;
...
uint8_t ThermalSensor_Bus2::setMax(float maxTemp)
{
...
uint8_t ThermalSensor_Bus2::setMin(float minTemp)
{
	// Convert the unit-ed value to a raw ADC value:
...
uint8_t ThermalSensor_Bus2::setEmissivity(float emis)
{
...
float ThermalSensor_Bus2::readEmissivity(void)
{
	int16_t ke;
...
uint8_t ThermalSensor_Bus2::readAddress(void)
{
	int16_t tempAdd;
...
uint8_t ThermalSensor_Bus2::setAddress(uint8_t newAdd)
{
	int16_t tempAdd;
...
uint8_t ThermalSensor_Bus2::readID(void)
{	
	for (int i=0; i<4; i++)
	{
		int16_t temp = 0;
...
uint8_t ThermalSensor_Bus2::wake(void)
{
...
int16_t ThermalSensor_Bus2::calcRawTemp(float calcTemp)
{
...
float ThermalSensor_Bus2::calcTemperature(int16_t rawTemp)
{
	float retTemp;
...
uint8_t ThermalSensor_Bus2::I2CReadWord(byte reg, int16_t * dest)
{
	Wire1.beginTransmission(_deviceAddress);
	Wire1.write(reg);
...
uint8_t ThermalSensor_Bus2::writeEEPROM(byte reg, int16_t data)
{	
...
uint8_t ThermalSensor_Bus2::I2CWriteWord(byte reg, int16_t data)
{
	uint8_t crc;
...
uint8_t ThermalSensor_Bus2::crc8 (uint8_t inCrc, uint8_t inData)
{
	uint8_t i;
	uint8_t data;
	data = inCrc ^ inData;
