...
ThermalSensor_Bus1::ThermalSensor_Bus1()
{
...
uint8_t ThermalSensor_Bus1::begin(uint8_t address)
{
...
void ThermalSensor_Bus1::setUnit(temperature_units unit)
{
	_defaultUnit = unit; // Store the unit into a private member
}

uint8_t ThermalSensor_Bus1::read()
{
...
uint8_t ThermalSensor_Bus1::readRange()
{
...
uint8_t ThermalSensor_Bus1::readObject2()
{
	int16_t rawObj;
...
uint8_t ThermalSensor_Bus1::readAmbient()
{
	int16_t rawAmb;
...
uint8_t ThermalSensor_Bus1::readMax()
{
	int16_t toMax;
...
uint8_t ThermalSensor_Bus1::readMin()
{
	int16_t toMin;
...
uint8_t ThermalSensor_Bus1::setMax(float maxTemp)
{
...
uint8_t ThermalSensor_Bus1::setMin(float minTemp)
{
	// Convert the unit-ed value to a raw ADC value:
...
uint8_t ThermalSensor_Bus1::setEmissivity(float emis)
{
...
float ThermalSensor_Bus1::readEmissivity(void)
{
	int16_t ke;
...
uint8_t ThermalSensor_Bus1::readAddress(void)
{
	int16_t tempAdd;
...
uint8_t ThermalSensor_Bus1::setAddress(uint8_t newAdd)
{
	int16_t tempAdd;
...
uint8_t ThermalSensor_Bus1::readID(void)
{	
	for (int i=0; i<4; i++)
	{
		int16_t temp = 0;
...
uint8_t ThermalSensor_Bus1::sleep(void)
{
	...
	Wire1.write(MLX90614_REGISTER_SLEEP);
	Wire1.write(crc);
...
uint8_t ThermalSensor_Bus1::wake(void)
{
	// Wake operation from datasheet
	Wire1.endTransmission(true); // stop i2c bus to send wake up request via digital pins
...
int16_t ThermalSensor_Bus1::calcRawTemp(float calcTemp)
{
...
float ThermalSensor_Bus1::calcTemperature(int16_t rawTemp)
{
	float retTemp;
...
uint8_t ThermalSensor_Bus1::I2CReadWord(byte reg, int16_t * dest)
{
	Wire1.beginTransmission(_deviceAddress);
	Wire1.write(reg);
	
	Wire1.endTransmission(false); // Send restart
	Wire1.requestFrom(_deviceAddress, (uint8_t) 3);
...
uint8_t ThermalSensor_Bus1::writeEEPROM(byte reg, int16_t data)
{	
...
uint8_t ThermalSensor_Bus1::I2CWriteWord(byte reg, int16_t data)
{
	uint8_t crc;
...
uint8_t ThermalSensor_Bus1::crc8 (uint8_t inCrc, uint8_t inData)
{
	uint8_t i;
	uint8_t data;
	data = inCrc ^ inData;
